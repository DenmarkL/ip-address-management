    ...
    <Dialog v-model:visible="showDialog" header="Add New IP" modal class="w-1/3">
      <div class="flex flex-col gap-4">
        ...
        <div>
          ...
          <Dropdown v-model="newIP.ip_type" :options="ipTypes" class="w-full mt-1" placeholder="Select IP Type"/>
        </div>
...
const fetchIPAddresses = async () => {
  try {
    const response = await api.get('/ip-addresses'); // No need to manually add Authorization
    ipAddresses.value = response.data.data; // Ensure correct data structure
  } catch (error) {
...
const addIPAddress = async () => {
    try {
        const response = await api.post('/ip-addresses', newIP.value);
        // Push the correct data to the table
        ipAddresses.value.push(response.data.data);
        // Close modal and reset form
        showDialog.value = false;
        newIP.value = { ip_address: '', ip_type: null, label: '', comment: '' };
